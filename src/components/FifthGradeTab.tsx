...
const FifthGradeContainer = styled.div`
  padding: 1.5rem;
  max-width: 1200px;
  margin: 0 auto;
...
const FifthGradeTab: React.FC = () => {
  const [allCardsFlipped, setAllCardsFlipped] = useState(false);

  // Get Fifth grade words
  const fifthGradeCategory = words.categories.find(category => category.name === "Fifth grade");
  const fifthGradeWords = fifthGradeCategory?.wordPairs || [];

  return (
    <FifthGradeContainer>
      <Title>Fifth Grade Words 🎓</Title>
      <Instructions>
      ...
      <CardGrid>
        {fifthGradeWords.map((pair, index) => (
          <WordCard
            key={index}
